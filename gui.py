...
from file_move import get_mounted_drives
import colour
...
class App(ctk.CTk):
	def __init__(self):
		...
		# Create buttons for up to 3 drives
		for i in range(3):
			drive_text = drives[i] if i < len(drives) else "No drive found"
			drive_button = ctk.CTkButton(tab_frame_left, text=f"Move files for {drive_text}", command=user_dir)
			drive_button.pack(side="top", pady=10)
			drive_button.configure(fg_color=colour.BLUE, text_color=colour.OFF_WHITE)
